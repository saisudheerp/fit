...
import { getRoutines, createRoutine, updateRoutine, deleteRoutine, duplicateRoutine, getExercises, createWorkoutSession, logExercise, saveWorkoutProgress, getAllWorkoutProgress, deleteWorkoutProgress, subscribeToWorkoutProgress, deleteAllOldWorkoutProgress } from '../lib/firebase-database';
...
export default function Routines() {
  ...
  useEffect(() => {
    if (user) {
      ...
      deleteAllOldWorkoutProgress(user.uid).catch(err => console.error('Error cleaning old progress:', err));
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToWorkoutProgress(user.uid, (progress) => {
      if (progress.length > 0) {
        ...
        const progressWithRoutines = progress.map(p => {
          const matchingRoutine = routines.find(r => r.name === p.routineName);
          if (matchingRoutine) {
  ...
  const handleDeleteAllRoutines = async () => {
    if (!confirm(`Are you sure you want to delete ALL ${routines.length} routines? This cannot be undone!`)) return;
  ...
  const handleGeneratePredefined = async (type) => {
    ...
    const exercisesPerGroup = Math.max(1, Math.floor(6 / config.muscleGroups.length));
  ...
  const routineColors = [
    { gradient: 'linear-gradient(135deg, #FF6B6B 0%, #FF8E53 100%)', color: '#FF6B6B', glow: 'rgba(255, 107, 107, 0.4)' },
    { gradient: 'linear-gradient(135deg, #4ECDC4 0%, #44A08D 100%)', color: '#4ECDC4', glow: 'rgba(78, 205, 196, 0.4)' },
    { gradient: 'linear-gradient(135deg, #FFD93D 0%, #F5C000 100%)', color: '#FFD93D', glow: 'rgba(255, 217, 61, 0.4)' },
    { gradient: 'linear-gradient(135deg, #A8E6CF 0%, #88D8B0 100%)', color: '#A8E6CF', glow: 'rgba(168, 230, 207, 0.4)' },
    { gradient: 'linear-gradient(135deg, #FFB6B9 0%, #FFA5A8 100%)', color: '#FFB6B9', glow: 'rgba(255, 182, 185, 0.4)' },
    { gradient: 'linear-gradient(135deg, #B19CD9 0%, #9B7EC4 100%)', color: '#B19CD9', glow: 'rgba(177, 156, 217, 0.4)' }
  ];

  if (loading) {
    return (
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 24px', textAlign: 'center' }}>
        <span className="material-icons rotating" style={{ fontSize: '48px', color: '#666' }}>sync</span>
    ...
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 24px' }}>
      <div style={{ marginBottom: '40px', animation: 'fadeIn 0.5s ease-out' }}>
        <h2 style={{
          ...
          marginBottom: '8px',
          background: 'linear-gradient(135deg, #A8E6CF 0%, #4ECDC4 35%, #88D8B0 70%, #66D9A5 100%)',
          WebkitBackgroundClip: 'text',
          WebkitTextFillColor: 'transparent',
          backgroundClip: 'text'
        }}>WORKOUT ROUTINES</h2>
        <p style={{ color: '#999', fontSize: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span className="material-icons" style={{ fontSize: '20px', color: '#A8E6CF' }}>fitness_center</span>
      ...
      {savedWorkout && Array.isArray(savedWorkout) && savedWorkout.length > 0 && (
        <div style={{ marginBottom: '48px' }}>
          <h3 style={{
            ...
            <span className="material-icons" style={{ fontSize: '32px' }}>play_circle</span>
            WORKOUTS IN PROGRESS
          </h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '16px' }}>
            {savedWorkout.map((workout, idx) => workout.routine && (
              <div key={idx} style={{
                background: 'linear-gradient(135deg, rgba(168, 230, 207, 0.1) 0%, rgba(78, 205, 196, 0.1) 100%)',
                ...
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                    <span className="material-icons" style={{ fontSize: '24px', color: '#A8E6CF' }}>play_circle</span>
                    <h4 style={{
                      fontSize: '18px',
                      fontWeight: 700,
                      color: '#fff',
                      fontFamily: 'Bebas Neue, Impact, sans-serif',
                      letterSpacing: '0.05em'
                    }}>{workout.routineName || workout.routine.name}</h4>
                  </div>
                  <p style={{ fontSize: '13px', color: '#999' }}>
                    Exercise {(workout.currentExerciseIndex || 0) + 1} of {workout.routine.exercises?.length || 0} • 
                    Set {workout.currentSet || 1} • 
                ...
                onMouseEnter={(e) => {
                  ...
                  e.currentTarget.style.boxShadow = '0 6px 20px rgba(168, 230, 207, 0.7)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.transform = 'translateY(0)';
                  e.currentTarget.style.boxShadow = '0 4px 15px rgba(168, 230, 207, 0.5)';
                }}>
      ...
      <div>
        <h3 style={{
          ...
          <span className="material-icons" style={{ fontSize: '32px' }}>add_circle</span>
          MY ROUTINES
        </h3>
      ...
      {/* Debug Info */}
      <div style={{ position: 'fixed', bottom: '10px', right: '10px', background: '#1a1a1a', padding: '10px', borderRadius: '8px', fontSize: '10px', color: '#fff', border: '1px solid #4ECDC4', zIndex: 9999, display: 'none' }}>
        Create: {String(showCreateModal)} | Edit: {String(!!editingRoutine)} | Gen: {String(showGeneratorModal)} | View: {String(!!selectedRoutine)}
      </div>
    </div>
  );
}
...
function EmptyState() {
  return (
    <div style={{
      ...
      <span className="material-icons" style={{ fontSize: '96px', color: '#2a2a2a', marginBottom: '24px' }}>
        fitness_center
      </span>
      <p style={{ color: '#999', fontSize: '18px', marginBottom: '12px' }}>No routines yet</p>
      <p style={{ color: '#666', fontSize: '14px' }}>Create a custom routine or generate one automatically</p>
    </div>
  );
}

function RoutineCard({ routine, colors, onView, onEdit, onDelete, onDuplicate, bodyWeight }) {
  const { totalCalories, totalDuration } = calculateRoutineStats(routine.exercises || [], bodyWeight);
  ...
  return (
    ...
    onMouseLeave={(e) => {
      ...
      <div style={{ marginBottom: '16px', paddingLeft: '8px' }} onClick={onView}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
          <div style={{
            width: '40px',
            height: '40px',
            ...
            <span className="material-icons" style={{ fontSize: '24px', color: '#fff' }}>
              {routine.type === 'home' ? 'home' : routine.type === 'gym' ? 'fitness_center' : 'edit'}
            </span>
          </div>
          <div style={{ flex: 1 }}>
            <h3 style={{
...
function calculateRoutineStats(exercises, bodyWeight) {
  ...
  exercises.forEach(ex => {
    ...
    const isTimed = exerciseData.type === 'timed' || exerciseData.type === 'cardio';
    ...
    const calories = calculateExerciseCalories(
      { type: exerciseData.type, met: exerciseData.met, volumeCoefficient: exerciseData.volume_coefficient },
      { 
...
function RoutineModal({ routine, exercises, onClose, onSave, bodyWeight }) {
  ...
  const { totalCalories, totalDuration } = calculateRoutineStats(selectedExercises, bodyWeight);
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <h3 style={{
            ...
            WebkitBackgroundClip: 'text',
            WebkitTextFillColor: 'transparent'
          }}>{routine ? 'EDIT ROUTINE' : 'CREATE ROUTINE'}</h3>
        ...
        <div style={{ padding: '24px', overflowY: 'auto', flex: 1 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '24px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Routine Name</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Type</label>
              <select
                value={type}
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Split</label>
              <select
                value={split}
          ...
          {selectedExercises.length > 0 && (
            <div style={{ marginBottom: '24px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h4 style={{ fontSize: '14px', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Selected Exercises ({selectedExercises.length})</h4>
                ...
                {selectedExercises.map((ex, index) => (
                  <div key={index} style={{
                    ...
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: ex.exerciseData?.type === 'timed' || ex.exerciseData?.type === 'cardio' ? 'repeat(3, 1fr)' : 'repeat(4, 1fr)', gap: '12px' }}>
                        {(ex.exerciseData?.type === 'timed' || ex.exerciseData?.type === 'cardio') ? (
                          <>
                            <InputField label="Sets" value={ex.sets} onChange={(v) => handleUpdateExercise(index, 'sets', v)} />
                            <InputField label="Duration (s)" value={ex.duration_seconds || 60} onChange={(v) => handleUpdateExercise(index, 'duration_seconds', v)} />
                            <InputField label="Rest (s)" value={ex.rest_seconds} onChange={(v) => handleUpdateExercise(index, 'rest_seconds', v)} />
                          </>
                        ) : (
                          <>
                            <InputField label="Sets" value={ex.sets} onChange={(v) => handleUpdateExercise(index, 'sets', v)} />
                            <InputField label="Reps" value={ex.reps} onChange={(v) => handleUpdateExercise(index, 'reps', v)} />
                            <InputField label="Weight (kg)" value={ex.weight} onChange={(v) => handleUpdateExercise(index, 'weight', v)} />
                            <InputField label="Rest (s)" value={ex.rest_seconds} onChange={(v) => handleUpdateExercise(index, 'rest_seconds', v)} />
                          </>
                        )}
                      </div>
                    </div>
                    <button onClick={() => handleRemoveExercise(index)} style={{
                      background: 'none',
                      border: 'none',
                      color: '#FF6B6B',
                      cursor: 'pointer',
          ...
          <div>
            <h4 style={{ fontSize: '14px', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>Add Exercises</h4>
            <input
              type="text"
              value={searchTerm}
            ...
            <div style={{ maxHeight: '200px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {filteredExercises.slice(0, 10).map(ex => (
                <div key={ex.id} onClick={() => handleAddExercise(ex)} style={{
                  padding: '12px',
                  background: '#0a0a0a',
                  border: '2px solid #2a2a2a',
                  borderRadius: '8px',
                ...
                onMouseLeave={(e) => {
                  ...
                  <div>
                    <div style={{ fontSize: '13px', color: '#fff', fontWeight: 600 }}>{ex.name}</div>
                    <div style={{ fontSize: '11px', color: '#666' }}>{ex.body_part}</div>
                  </div>
                  <span className="material-icons" style={{ color: '#4ECDC4', fontSize: '20px' }}>add_circle</span>
                </div>
              ))}
            </div>
          </div>
...
function InputField({ label, value, onChange }) {
  return (
    <div>
      <label style={{ display: 'block', fontSize: '10px', color: '#666', marginBottom: '4px', textTransform: 'uppercase' }}>{label}</label>
      <input
        type="number"
        value={value}
...
function GeneratorModal({ exercises, onClose, onGenerate, bodyWeight }) {
  ...
  const handleGenerate = () => {
    const generatedRoutine = generateRoutine(exercises, goal, experience, equipment, split);
    ...
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div>
            <h3 style={{
              ...
              WebkitTextFillColor: 'transparent',
              marginBottom: '4px'
            }}>AI ROUTINE GENERATOR</h3>
            <p style={{ fontSize: '12px', color: '#666' }}>Auto-create routines based on your goals</p>
          </div>
          <button onClick={onClose} style={{
            background: 'none',
            border: 'none',
        ...
        <div style={{ padding: '24px' }}>
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Primary Goal</label>
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Experience Level</label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              {[
                { value: 'beginner', label: 'Beginner', icon: 'star_outline' },
                { value: 'intermediate', label: 'Intermediate', icon: 'star_half' },
                { value: 'advanced', label: 'Advanced', icon: 'star' }
              ].map(opt => (
                <button key={opt.value} onClick={() => setExperience(opt.value)} style={{
                  padding: '12px',
                  background: experience === opt.value ? 'linear-gradient(135deg, #FFD93D 0%, #F5C000 100%)' : '#0a0a0a',
                  border: experience === opt.value ? 'none' : '2px solid #2a2a2a',
                  borderRadius: '12px',
                  ...
                  <span className="material-icons" style={{ fontSize: '20px' }}>{opt.icon}</span>
                  {opt.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Equipment Available</label>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
              {[
                ...
                <button key={opt.value} onClick={() => setEquipment(opt.value)} style={{
                  padding: '12px',
                  background: equipment === opt.value ? 'linear-gradient(135deg, #A8E6CF 0%, #88D8B0 100%)' : '#0a0a0a',
                  border: equipment === opt.value ? 'none' : '2px solid #2a2a2a',
                  borderRadius: '12px',
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '12px', color: '#999', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Workout Split</label>
            <select
              value={split}
              onChange={(e) => setSplit(e.target.value)}
              style={{
...
function OptionCard({ icon, label, description, selected, onClick, color }) {
  return (
    <button onClick={onClick} style={{
      padding: '16px',
      background: selected ? `linear-gradient(135deg, ${color} 0%, ${color}dd 100%)` : '#0a0a0a',
      ...
      <span className="material-icons" style={{ fontSize: '32px', color: selected ? '#fff' : color }}>{icon}</span>
...
// View Routine Modal
function ViewRoutineModal({ routine, exercises, onClose, onEdit, onStartWorkout, bodyWeight }) {
  const { totalCalories, totalDuration } = calculateRoutineStats(routine.exercises || [], bodyWeight);
    ...
    <div style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
            <div style={{
              ...
              <div style={{ fontSize: '24px', fontWeight: 700, fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                {(routine.exercises || []).length}
              </div>
              <div style={{ fontSize: '11px', opacity: 0.9, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Exercises</div>
            </div>
            <div style={{
              background: 'linear-gradient(135deg, #4ECDC4 0%, #44A08D 100%)',
              padding: '12px',
              ...
              <div style={{ fontSize: '11px', opacity: 0.9, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Duration</div>
            </div>
            <div style={{
              ...
              <div style={{ fontSize: '11px', opacity: 0.9, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Calories</div>
            </div>
          </div>
        </div>

        <div style={{ padding: '24px', overflowY: 'auto', flex: 1 }}>
          <h4 style={{ fontSize: '14px', color: '#999', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '16px' }}>
              ...
              <div key={index} style={{
                ...
                <div style={{ paddingLeft: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <div>
                      <div style={{ fontSize: '14px', fontWeight: 600, color: '#fff' }}>
                        {index + 1}. {ex.exerciseData?.name || 'Unknown Exercise'}
                      </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '18px', fontWeight: 700, color: '#4ECDC4', fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                        {ex.sets}
                      </div>
                      <div style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase' }}>Sets</div>
                    </div>
                    {(ex.exerciseData?.type === 'timed' || ex.exerciseData?.type === 'cardio') ? (
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '18px', fontWeight: 700, color: '#FFD93D', fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                          {ex.duration_seconds || 60}s
                        </div>
                        <div style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase' }}>Duration</div>
                      </div>
                    ) : (
                      <>
                        <div style={{ textAlign: 'center' }}>
                        ...
                        <div style={{ textAlign: 'center' }}>
                          <div style={{ fontSize: '18px', fontWeight: 700, color: '#FF6B6B', fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                            {ex.weight}kg
                          </div>
                    ...
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '18px', fontWeight: 700, color: '#A8E6CF', fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                        {ex.rest_seconds}s
                      </div>
                      <div style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase' }}>Rest</div>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div style={{
          ...
          <button onClick={() => onStartWorkout(routine)} style={{
            flex: 1,
            padding: '14px',
            background: 'linear-gradient(135deg, #FF6B6B 0%, #FF8E53 100%)',
            border: 'none',
...
function generateRoutine(exercises, goal, experience, equipment, split) {
  const muscleGroups = {
    full_body: ['chest', 'back', 'shoulders', 'biceps', 'triceps', 'legs', 'core'],
    upper_lower: split === 'upper' ? ['chest', 'back', 'shoulders', 'biceps', 'triceps'] : ['legs', 'core'],
    push_pull_legs: split === 'push' ? ['chest', 'shoulders', 'triceps'] : split === 'pull' ? ['back', 'biceps'] : ['legs', 'core']
  };
  ...
  const routineName = `${experience.charAt(0).toUpperCase() + experience.slice(1)} ${
    split === 'full_body' ? 'Full Body' : 
    split === 'upper_lower' ? 'Upper/Lower' : 'PPL'
  } - ${goal === 'fat_loss' ? 'Fat Loss' : 'Muscle Gain'}`;

  return {
    name: routineName,
    type: equipment,
    split: split === 'full_body' ? 'Full Body' : split === 'upper_lower' ? 'Upper/Lower' : 'PPL',
    exercises: selectedExercises
  };
}
...
function ActiveWorkoutModal({ workout, user, profile, onClose, onComplete }) {
  ...
  }, [sessionId, currentExerciseIndex, currentSet, completedSets, exerciseStates, skippedSets]);
  ...
  const handleSkipExercise = async () => {
    if (!confirm(`Skip all remaining sets of ${currentExercise?.exerciseData?.name}?`)) return;
    ...
    const completedSetsForExercise = completedSets.filter(s => s.exerciseIndex === currentExerciseIndex);
    const skippedSetsForExercise = newSkippedSets.filter(s => s.exerciseIndex === currentExerciseIndex);
    
    if (completedSetsForExercise.length > 0) {
      try {
        const exercise = currentExercise;
        const isTimed = exercise.exerciseData?.type === 'timed' || exercise.exerciseData?.type === 'cardio';
  ...
  const handleCompleteSet = async () => {
    ...
    if (currentSet < currentExercise.sets) {
      ...
      try {
        ...
        const completedSetsForExercise = completedSets.filter(s => s.exerciseIndex === currentExerciseIndex);
        const skippedSetsForExercise = skippedSets.filter(s => s.exerciseIndex === currentExerciseIndex);
  ...
  const isTimed = currentExercise?.exerciseData?.type === 'timed' || currentExercise?.exerciseData?.type === 'cardio';
  ...
  const calculateLiveStats = () => {
    ...
    workout.routine.exercises.forEach((ex, idx) => {
      ...
      const exerciseIsTimed = ex.exerciseData?.type === 'timed' || ex.exerciseData?.type === 'cardio';
      
      // Calculate completed sets for this exercise
      const completedSetsForExercise = completedSets.filter(s => s.exerciseIndex === idx).length;
      const skippedSetsForExercise = skippedSets.filter(s => s.exerciseIndex === idx).length;
      const effectiveSets = idx < currentExerciseIndex 
        ? ex.sets 
        : (idx === currentExerciseIndex ? completedSetsForExercise : 0);
      ...
      totalDuration += exerciseDuration + (effectiveSets * (ex.rest_seconds / 60));
    });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          padding: '24px',
          borderBottom: '2px solid #2a2a2a',
          background: isPaused ? 'linear-gradient(135deg, #FF6B6B 0%, #C92A2A 100%)' : 'transparent'
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
            <div>
              ...
              <p style={{ fontSize: '16px', color: '#4ECDC4', fontWeight: 600, marginBottom: '4px' }}>
                {workout.routine.name}
              </p>
              <p style={{ fontSize: '12px', color: '#666' }}>
                {completedExercises} completed • {remainingExercises} remaining • {progress}% done
              </p>
            </div>
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <button onClick={togglePause} style={{
                background: isPaused ? 'linear-gradient(135deg, #4ECDC4 0%, #44A08D 100%)' : '#2a2a2a',
                border: 'none',
                color: '#fff',
                cursor: 'pointer',
                fontSize: '20px',
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#FF6B6B' }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: 600 }}>
                  Paused at Set {currentSet} of {currentExercise?.exerciseData?.name}
                </span>
              </div>
              <p style={{ fontSize: '12px', color: '#999', marginTop: '4px', marginLeft: '32px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginTop: '16px' }}>
            <div style={{
              ...
              <div style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase' }}>Calories</div>
            </div>
            <div style={{
              background: '#0a0a0a',
              border: '1px solid #2a2a2a',
              ...
              <div style={{ fontSize: '10px', color: '#666', textTransform: 'uppercase' }}>Volume kg</div>
            </div>
            <div style={{
              background: '#0a0a0a',
              border: '1px solid #2a2a2a',
        ...
        <div style={{ padding: '24px' }}>
          <div style={{
            ...
            <h4 style={{
              ...
              marginBottom: '8px',
              textAlign: 'center'
            }}>{currentExercise?.exerciseData?.name || 'Exercise'}</h4>
            <p style={{ fontSize: '12px', color: '#666', textAlign: 'center', marginBottom: '24px' }}>
              {currentExercise?.exerciseData?.body_part} • {currentExercise?.exerciseData?.type}
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: isTimed ? '1fr 1fr' : '1fr 1fr 1fr', gap: '16px', marginBottom: '24px' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '48px', fontWeight: 700, color: '#FF6B6B', fontFamily: 'Bebas Neue, Impact, sans-serif' }}>
                  {currentSet}/{currentExercise?.sets}
                </div>
                <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>Set</div>
              </div>
              {isTimed ? (
                <div style={{ textAlign: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '4px' }}>
                    <button onClick={() => adjustTime(-10)} style={{
                      background: '#2a2a2a',
                      border: 'none',
                      color: '#4ECDC4',
                      cursor: 'pointer',
                    ...
                    <div style={{ fontSize: '48px', fontWeight: 700, color: '#4ECDC4', fontFamily: 'Bebas Neue, Impact, sans-serif', minWidth: '120px' }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>Duration (±10s)</div>
                </div>
              ) : (
                <>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '4px' }}>
                      ...
                      <div style={{ fontSize: '48px', fontWeight: 700, color: '#4ECDC4', fontFamily: 'Bebas Neue, Impact, sans-serif', minWidth: '80px' }}>
                        {currentState.reps}
                      </div>
                      <button onClick={() => adjustReps(1)} style={{
                        background: '#2a2a2a',
                    ...
                    <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>Reps (±1)</div>
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '4px' }}>
                      ...
                      <div style={{ fontSize: '48px', fontWeight: 700, color: '#FFD93D', fontFamily: 'Bebas Neue, Impact, sans-serif', minWidth: '120px' }}>
                    ...
                    <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>Weight (±2.5kg)</div>
                  </div>
                </>
              )}
            </div>
              ...
              <div style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span className="material-icons" style={{ fontSize: '20px', color: '#667eea' }}>auto_awesome</span>
                    <div>
                      <div style={{ fontSize: '12px', color: '#999', marginBottom: '2px' }}>Suggested progression</div>
                      <div style={{ fontSize: '14px', color: '#fff', fontWeight: 600 }}>
                        {currentState.weight > 0 ? `${currentState.weight + 2.5}kg` : `${currentState.reps + 1} reps`}
                      </div>
                    </div>
                  </div>
                  <button onClick={() => {
            ...
            <div style={{ textAlign: 'center', padding: '16px', background: '#0a0a0a', borderRadius: '12px' }}>
              <span className="material-icons" style={{ fontSize: '24px', color: '#A8E6CF', marginBottom: '8px' }}>
                timer
              </span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {/* Skip buttons row */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              <button onClick={handleSkipSet} style={{
                padding: '12px',
                background: '#2a2a2a',
                border: 'none',
                borderRadius: '12px',
                ...
                <span className="material-icons" style={{ fontSize: '18px' }}>skip_next</span>
                Skip Set
              </button>
              <button onClick={handleSkipExercise} style={{
                padding: '12px',
                background: '#2a2a2a',
                border: 'none',
                borderRadius: '12px',
                ...
                <span className="material-icons" style={{ fontSize: '18px' }}>fast_forward</span>
                Skip Exercise
              </button>
            </div>

            {/* Complete set button */}
            <button onClick={handleCompleteSet} disabled={isPaused} style={{
              padding: '18px',
              background: isPaused ? '#444' : 'linear-gradient(135deg, #4ECDC4 0%, #44A08D 100%)',
              border: 'none',
              borderRadius: '12px',
              color: '#fff',
              fontSize: '16px',
              ...
              textTransform: 'uppercase',
              letterSpacing: '0.05em',
              boxShadow: isPaused ? 'none' : '0 4px 20px rgba(78, 205, 196, 0.4)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              gap: '8px',
  ...
  const predefinedRoutines = [
    {
      ...
      exercises: [
        ...
        { name: 'Farmer Carry', sets: 3, reps: 0, rest: 90, duration_seconds: 30 },
    ...
    },
    {
      ...
      exercises: [
        ...
        { name: 'Plank', sets: 3, reps: 0, rest: 90, duration_seconds: 60 },
        { name: 'Russian Twist', sets: 3, reps: 20, rest: 90 },
        { name: 'Mountain Climbers', sets: 3, reps: 0, rest: 90, duration_seconds: 30 },
    ...
    },
    {
      ...
      exercises: [
        ...
        { name: 'Rowing Machine', sets: 1, reps: 0, rest: 0, duration_seconds: 900 },
            ...
            {predefinedRoutines.map((routine, index) => {
              ...
              return (
                <div key={index} style={{
                  ...
                  borderRadius: '20px',
                  overflow: 'hidden',
                  border: `2px solid ${isExpanded ? routine.gradient.match(/#[0-9A-F]{6}/i)?.[0] : 'rgba(255, 255, 255, 0.05)'}`,
                  transition: 'all 0.3s ease',
                  boxShadow: isExpanded ? `0 12px 40px ${routine.glow}` : '0 4px 12px rgba(0, 0, 0, 0.3)'
                }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ position: 'relative', zIndex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                        <span className="material-icons" style={{ fontSize: '32px', color: '#fff' }}>
                          {routine.icon}
                        </span>
                        <h3 style={{
                          margin: 0,
                  ...
                  <div style={{ padding: '20px' }}>
                    ...
                    <div style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#999' }}>
                        <span className="material-icons" style={{ fontSize: '18px', color: routine.gradient.match(/#[0-9A-F]{6}/i)?.[0] }}>fitness_center</span>
                        <span style={{ fontSize: '14px', fontWeight: 600 }}>{routine.exercises.length} exercises</span>
                      </div>
                    </div>
                      ...
                      {routine.exercises.map((ex, i) => (
                        <div key={i} style={{
                          ...
                          <span style={{ fontSize: '12px', color: '#666', fontWeight: 600 }}>
                            {ex.duration_seconds ? `${ex.sets}×${ex.duration_seconds}s` : `${ex.sets}×${ex.reps}`}
                          </span>
                        </div>
                      ))}
                    </div>
