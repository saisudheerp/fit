...
export default function Dashboard() {
  ...
  const statsCards = [
    { icon: 'local_fire_department', label: 'Calories Burned', value: stats.totalCalories.toFixed(0), unit: 'kcal', gradient: 'linear-gradient(135deg, #FF6B6B 0%, #FF8E53 100%)', color: '#FF6B6B', glow: 'rgba(255, 107, 107, 0.3)' },
    { icon: 'directions_walk', label: 'Steps', value: stats.totalSteps.toFixed(0), unit: 'steps', gradient: 'linear-gradient(135deg, #4ECDC4 0%, #44A08D 100%)', color: '#4ECDC4', glow: 'rgba(78, 205, 196, 0.3)' },
    { icon: 'timeline', label: 'Workouts Today', value: stats.workoutsToday.toString(), unit: 'sessions', gradient: 'linear-gradient(135deg, #A8E6CF 0%, #88D8B0 100%)', color: '#A8E6CF', glow: 'rgba(168, 230, 207, 0.3)' }
  ];
  ...
  return (
    <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 24px' }}>
      ...
      <div style={{ marginBottom: '48px', animation: 'fadeIn 0.5s ease-out' }}>
        ...
        }}>{profile?.name ? `WELCOME, ${profile.name.toUpperCase()}` : 'DASHBOARD'}</h2>
        <p style={{ color: '#999', fontSize: '16px', fontWeight: 500, display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span className="material-icons pulse" style={{ fontSize: '20px', color: '#A8E6CF' }}>local_fire_department</span>
        ...
        {statsCards.map((stat, idx) => (
          ...
          onMouseLeave={(e) => {
            ...
            <div style={{ 
              ...
              <span className="material-icons" style={{ fontSize: '32px', color: stat.color }}>{stat.icon}</span>
              <div style={{
                fontSize: '11px',
                color: '#666',
      ...
      <div style={{
        ...
        <Panel title="Recent Activity" icon="history">
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px', color: '#666' }}>
              <span className="material-icons pulse" style={{ fontSize: '64px', marginBottom: '16px', opacity: 0.3 }}>fitness_center</span>
              <p style={{ fontSize: '16px', marginBottom: '8px', color: '#999' }}>No recent workouts</p>
              <p style={{ fontSize: '13px', marginBottom: '24px' }}>Start tracking your exercises</p>
              <a href="/log" style={{
                display: 'inline-flex',
                alignItems: 'center',
                gap: '8px',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {recentLogs.map((log, idx) => (
                <div key={idx} className="fade-in" style={{
                  padding: '20px',
                  background: 'linear-gradient(90deg, #FFB6B908 0%, #0a0a0a 15%)',
                  borderRadius: '16px',
                ...
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = 'linear-gradient(90deg, #FFB6B920 0%, #FFB6B908 15%)';
                  e.currentTarget.style.borderLeftColor = '#FFB6B9';
                  e.currentTarget.style.transform = 'translateX(8px)';
                  e.currentTarget.style.boxShadow = '0 4px 20px rgba(255, 182, 185, 0.25)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = 'linear-gradient(90deg, #FFB6B908 0%, #0a0a0a 15%)';
                  e.currentTarget.style.borderLeftColor = '#FFB6B9';
                  e.currentTarget.style.transform = 'translateX(0)';
                  e.currentTarget.style.boxShadow = '0 2px 10px rgba(255, 182, 185, 0.1)';
                }}>
                  <div style={{ 
                    fontSize: '16px', 
                    fontWeight: 700, 
                    ...
                    <span className="material-icons" style={{ fontSize: '20px', color: '#FFB6B9' }}>fitness_center</span>
                    {log.exerciseName}
                  </div>
                  <div style={{ 
                    fontSize: '13px', 
                    ...
                    {log.reps === 0 && log.durationSeconds 
                      ? `${log.sets} set${log.sets > 1 ? 's' : ''} × ${log.durationSeconds}s duration`
                      : `${log.sets} set${log.sets > 1 ? 's' : ''} × ${log.reps} reps @ ${log.weightKg}kg`
                    }
                  </div>
                </div>
              ))}
