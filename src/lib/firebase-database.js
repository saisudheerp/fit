...
export async function getExercises() {
  ...
  return querySnapshot.docs.map(doc => {
    ...
    else if (data.name.includes('Plank') || 
    ...
    else if (data.name.includes('Push-up') || 
    ...
    return {
      ...
      equipment: requiresWeight ? 'weights' : (type === 'cardio' || type === 'timed' ? type : 'bodyweight'),
      met: data.met_value,
...
export async function getExerciseLogs(userId, date) {
  ...
  const logsWithNames = await Promise.all(logs.map(async (log) => {
    try {
      ...
      return {
        ...log,
        exerciseName: exerciseDoc.exists() ? exerciseDoc.data().name : 'Unknown Exercise'
      };
    } catch (error) {
      return {
        ...log,
...
export async function logSteps(userId, stepsData) {
  ...
  await setDoc(doc(db, 'step_logs', docId), {
...
export async function getUserStats(userId) {
  ...
  const totalVolume = exerciseLogs.reduce((sum, log) => sum + (log.volume || 0), 0);
  const totalSteps = stepLogs.reduce((sum, log) => sum + (log.steps || 0), 0);
  
  console.log('getUserStats - totals:', { totalCalories, totalVolume, totalSteps, workoutsToday: exerciseLogs.length });
...
export async function saveWorkoutProgress(userId, routineName, progressData) {
  const docRef = doc(db, 'workout_progress', `${userId}_${routineName.replace(/\s+/g, '_')}`);
  await setDoc(docRef, {
    userId,
    routineName,
    ...progressData,
